...
<div class="col-md-12">
    <div class="tile">
        <a href="{{ route('admin.Pimage_paperCons') }}" class="btn btn-primary pull-right">+ Pimage</a>
        <h3>Product Image</h3>
        <div class="tile-body">
            <div style="overflow-x:auto;">
                <table class="table table-hover table-bordered" id="">
                    ...
                    @foreach($all_cons_pimage_info as $cons_pimage)
                    <tbody>
                        <tr>
                            <td>{{ $cons_pimage->id}}</td>
                            <td class="center"><img src='{{ asset($cons_pimage->image) }}'style="height: 80px; width: 80px;"></td>
                            
                            <td class="center">
                                @if($cons_pimage->publication_status==1)
                            ...
                            <td class="center">
                                @if($cons_pimage->publication_status==1)
                                <a class="btn btn-danger" href="{{ route('admin.unactive_Pimage_paperCons',$cons_pimage->id)}}">
                                    <i class="halflings-icon white thumbs-down"></i>
                                </a>
                                @else
                                <a class="btn btn-success" href="{{ route('admin.active_Pimage_paperCons',$cons_pimage->id) }}">
                                    <i class="halflings-icon white thumbs-up"></i>
                                </a>
                                @endif
                                <a class="btn btn-info" href="{{ route('admin.edit_Pimage_paperCons',$cons_pimage->id) }}">
                                    <i class="halflings-icon white edit"></i>
                                </a>
                                <a class="btn btn-danger" href="{{ route('admin.delete_Pimage_paperCons',$cons_pimage->id) }}" id="delete" >
