...
<div class="col-md-12">
    <div class="tile">
        <a href="{{ route('admin.Specification_paperCons') }}" class="btn btn-primary pull-right">+ Specification</a>
        <h3>Specification</h3>
        <div class="tile-body">
                ...
                <div class="table-responsive">
                    <table class="table table-hover table-bordered" id="">
                        ...
                        <tbody>
                            <tr>
                                <td>{{ $pdescribe->id}}</td>
                                <td>{{ $pdescribe->name}}</td>
                                <td>{{ $pdescribe->description}}</td>
                                <td class="center">
                                ...
                                <td class="center">
                                    @if($pdescribe->publication_status==1)
                                    <a class="btn btn-danger" href="{{ route('admin.unactive_Specification_paperCons',$pdescribe->id)}}">
                                        <i class="halflings-icon white thumbs-down"></i>
                                    </a>
                                    @else
                                    <a class="btn btn-success" href="{{ route('admin.active_Specification_paperCons',$pdescribe->id) }}">
                                        <i class="halflings-icon white thumbs-up"></i>
                                    </a>
                                    @endif
                                    <a class="btn btn-info" href="{{ route('admin.edit_Specification_paperCons',$pdescribe->id) }}">
                                        <i class="halflings-icon white edit"></i>
                                    </a>
                                    <a class="btn btn-danger" href="{{ route('admin.delete_Specification_paperCons',$pdescribe->id) }}" id="delete" >
